...
public class HeadersProcessor implements Processor {
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        Object fromHeader = in.getHeader("AssimblyFrom");
        Object toHeader = in.getHeader("AssimblyTo");
        if (fromHeader != null) {
            Language simple = exchange.getContext().resolveLanguage("simple");
            in.setHeader("AssimblyFromTimestamp", simple.createExpression(String.valueOf(System.currentTimeMillis())).evaluate(exchange, String.class));
        }
        if (toHeader != null) {
            Language simple = exchange.getContext().resolveLanguage("simple");
            in.setHeader("AssimblyToTimestamp", simple.createExpression(String.valueOf(System.currentTimeMillis())).evaluate(exchange, String.class));
        }
